...
void expect_equal(long exp, long real,const char* caller, int line);
#define EXPECT_EQUAL(exp, real) expect_equal(exp , real, __FILE__, __LINE__)
...
void expect_equal(long exp, long real, const char* caller, int line) {
    if (exp != real) {
        CTEST_WARN("%s:%d  expected %ld, got %ld", caller, line, exp, real);
    }
}
